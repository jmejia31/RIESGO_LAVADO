...
- Backend con modulos verticales para `Catalogos`, `Configuracion`, `Listas` y `MatricesRiesgos`; los modulos heredados restantes se conservan por tipo hasta que exista una fase funcional especifica para migrarlos.
...
|---|---:|---:|---|
| Backend | 5 | 15 | Calculo de matrices, Catalogos, Configuracion, manejo uniforme de errores, auditoria y limites HTTP/autorizacion de modulos |
...
|---|---|
| `dotnet test RIESGO_LAVADO.sln --configuration Release --no-restore` | 15/15 pruebas Backend |
...
- `Auth`, `Usuarios` y `Auditoria` conservan parte de la organizacion heredada del Backend; migrarlos exige fases funcionales separadas por su relacion con JWT, Active Directory y auditoria transaccional.
