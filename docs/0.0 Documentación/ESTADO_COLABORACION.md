...
> Actualización 2026-08-13 (Antigravity): **Serialización Completa de Fórmulas y Atributos Avanzados Certificada**. Se corrigió `serializarBuilderModelAJson` y `normalizarJsonABuilderModel` en `form-builder.models.ts` para mapear y conservar todas las propiedades de campo (`formula`, `opciones`, `codigoCatalogo`, `anchoColumnas`, `columnasPorFila`). Se integró el manejador `alCambiarPropiedadCampo()` con `(ngModelChange)` en todos los campos del Inspector. Pruebas Backend (319/319), Frontend (181/181) y Playwright E2E (13/13) **súperadas al 100%**. Publicado en `origin/desarrollo` (Commit `d5e84b9`).
