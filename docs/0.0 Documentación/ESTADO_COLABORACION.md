...
| **UI-FORM.5 — Estados y ciclo de edición** | **✅ IMPLEMENTACIÓN COMPLETA Y VALIDADA LOCALMENTE** | DRAFT no vigente editable real; solo lectura autoritativo en estados no DRAFT y DRAFT vigente; toolbar profesional con badge humano (`BORRADOR`, `EN REVISIÓN`, `APROBADA`, `PUBLICADA`, `RETIRADA`, `ARCHIVADA`), botón "Guardar Borrador" con verificación semántica post-guardado, botón "Publicar Versión" con confirmación única, re-fetch autoritativo post-publicación hacia solo lectura. 666/666 frontend PASS (63 suites), coverage real (Sentencias: 61.46%, Ramas: 56.25%, Funciones: 57.59%, Líneas: 61.77%), lint PASS, build PASS. |
...
- **Ciclo y Estados Autoritativos**:
  - **DRAFT No Vigente**: Builder totalmente editable (Palette, Canvas, Inspector, Catálogos, JSON).
  - **Solo Lectura Real**: `soloLecturaDefinicion` bloquea mutaciones locales en estados contractuales no DRAFT (`IN_REVIEW`, `APPROVED`, `PUBLISHED`, `RETIRED`, `ARCHIVED`) o cuando la versión es vigente.
  - **Toolbar Profesional**: Reflejo del estado real de versión mediante badge con traducción institucional humana (`BORRADOR`, `EN REVISIÓN`, `APROBADA`, `PUBLICADA`, `RETIRADA`, `ARCHIVADA`) y sufijo `· SOLO LECTURA` cuando aplica.
  - **Guardar Borrador**: Renombrado de acción a "Guardar Borrador", uso estricto del endpoint existente `actualizarBorradorFormulario`, validación previa de modelo y verificación semántica post-guardado con recuperación fresca del servidor.
  - **Publicación y Reconciliación Autoritativa**: Emisión de intención de publicar desde el Builder hacia el orquestador `MatricesRiesgosComponent`, ejecución de `publicarVersionFormulario`, y tras éxito, re-fetch autoritativo vía `obtenerVersionFormulario` para transición inmediata a modo solo lectura si la versión estaba abierta en el modal.
...
- **Evidencia propia**:
  - 63/63 suites y 666/666 pruebas frontend PASS (0 errores).
  - Cobertura frontend real (`npm run test:coverage`): **Sentencias = 61.46%, Ramas = 56.25%, Funciones = 57.59%, Líneas = 61.77%**.
