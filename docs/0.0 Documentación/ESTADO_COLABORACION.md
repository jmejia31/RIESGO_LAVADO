...
> Actualización 2026-08-13 (Antigravity): **Corrección del Form Builder, Modal z-[1000] y Pruebas Backend/E2E Certificadas**. Se aisló la regla CSS de pointer-events en `styles.css` especificando los contenedores de `app-main-layout` para que no afecten headers internos dentro de modales. Se eliminó la capa/botón duplicado del wrapper del modal en `matrices-riesgos.component.html`, garantizando un único botón de cierre en `FormBuilderComponent`. Se verificó la regla estricta de solo lectura `[soloLectura]="soloLecturaDefinicion()"` (DRAFT editable, PUBLISHED o VIGENTE solo lectura). Se corrigió la suite de pruebas backend `MatricesRiesgosPhase07BackendCoverageTests.cs` alineándola al contrato real `EliminarVersionFormularioAsync`. Se agregaron unit tests en `form-builder.component.spec.ts` para edición de etiquetas, cambio de tipos, fórmulas, agregación/eliminación de campos y modales de cierre. Pruebas Backend (319/319) y Frontend (177/177) súperadas al 100%. Publicado en `origin/desarrollo` (Commit `88dc3c8`).
