...
> Actualización 2026-08-18 (Antigravity): **F4.3 — CERTIFICACIÓN FUNCIONAL INTEGRAL DE F4 COMPLETADA**. Se validó la integración completa entre F3, F4.1 y F4.2 a través de la suite dedicada `matrices-riesgos.component.f4.3.spec.ts` (16/16 PASS). Se ajustó la cláusula WHERE en `MatricesRiesgosRepository.cs` (`AND NVL(f.FLU_ESTADO, 'BORRADOR') = :estado`) para alineación semántica del 100% con la proyección del SELECT. Pruebas automatizadas frontend Angular **335/335 (100%) PASS** (34 archivos spec), build Angular `dist/rl-app` **PASS**, Playwright E2E **14/14 (100%) PASS**, backend .NET Core **409/409 (100%) PASS**, Quality Gates **SUCCESS**. 0 modificaciones Oracle/SQL/DDL/DML. `main` intacta, PR #20 Draft. Publicado en `origin/desarrollo`.
