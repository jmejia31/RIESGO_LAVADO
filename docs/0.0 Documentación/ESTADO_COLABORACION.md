...
> Actualización 2026-08-17 (Antigravity): **F1 — Reproducción Funcional Autenticada (Matrices de Riesgos) Completada y Documentada**. Se realizó el recorrido funcional de las 12 secciones (F1-01 a F1-12) sobre el entorno verificado en F0/F0.1. Se aisló y tipificó con evidencia técnica el defecto de runtime `TypeError: this.evaluaciones(...).filter is not a function` en las tarjetas KPI, la concurrencia de llamadas paginadas (200 vs 20/10), el aislamiento de modales superpuestos `z-[1000]`, la carga de versiones históricas en edición dinámica y el desacoplamiento de señales de carga entre pestañas. Se elaboró la Matriz Maestra de Defectos F1 mapeando cada hallazgo a las fases correctivas (F2 a F12). 0 líneas de código productivo modificadas, 0 ejecuciones Oracle (DDL/DML = 0) y 0 modificaciones a pruebas. Publicado en `origin/desarrollo`.
