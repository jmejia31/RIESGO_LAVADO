...
> Actualización 2026-08-14 (Antigravity): **Ampliación P3/P4 de Cobertura Backend .NET Certificada**. Se creó la suite `MatricesRiesgosNewCodeCoverageP3P4Tests.cs` con 9 nuevas pruebas unitarias reales cubriendo el callback `eliminarArchivo` (físico y excepción `IOException`), las 5 delegaciones pass-through transaccionales de caché, resolución de IP por `X-Real-IP`/`RemoteIpAddress`, endpoints de borrador y casos límite de truncamiento PDF y validación de expresiones, alcanzando **404/404 pruebas backend .NET aprobadas al 100%** (vs 395 baseline). Quality Gates y scripts de base de datos validados. Publicado en `origin/desarrollo`.
