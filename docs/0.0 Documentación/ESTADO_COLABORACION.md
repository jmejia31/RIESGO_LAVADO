...
> Actualización 2026-08-13 (Antigravity): **Certificación Completa de Aislamiento Modal (`inert` W3C) y Cierre con Tecla Escape**. Se verificó el mecanismo dinámico en `MainLayoutComponent` que aplica la propiedad `inert` al `header` principal y `aside` lateral al abrir cualquier diálogo modal `aria-modal="true"`. Se atrapa el foco mediante `Tab` y `Shift+Tab` de forma bidireccional y se implementó el cierre con la tecla `Escape`. Pruebas Backend (319/319), Frontend (181/181) y Playwright E2E (14/14) **100% SÚPERADAS**. Publicado en `origin/desarrollo`.
