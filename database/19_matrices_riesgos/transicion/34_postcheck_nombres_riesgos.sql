-- Oracle 11g / READ ONLY. Ejecutar despu̩s de 33.
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('RISK_NAME_POSTCHECK_EXPECTED_SOURCE_ROWS=59');
  DBMS_OUTPUT.PUT_LINE('RISK_NAME_POSTCHECK_DUPLICATES=' || duplicates);
  DBMS_OUTPUT.PUT_LINE('RISK_NAME_POSTCHECK_DIFFS=' || diffs);
  DBMS_OUTPUT.PUT_LINE('RCUMP-COMPRAS-24_NAME=' || sample_name);
  DBMS_OUTPUT.PUT_LINE('RCUMP-COMPRAS-24_HAS_U+00F3=' || CASE WHEN INSTR(sample_name, UNISTR('\00F3')) > 0 THEN 'YES' ELSE 'NO' END);
  DBMS_OUTPUT.PUT_LINE('RCUMP-COMPRAS-24_HAS_U+00BF=' || CASE WHEN INSTR(sample_name, UNISTR('\00BF')) > 0 THEN 'YES' ELSE 'NO' END);
  DBMS_OUTPUT.PUT_LINE('RISK_NAME_POSTCHECK_STATUS=' || CASE WHEN db_rows=59 AND duplicates=0 AND diffs=0 AND sample_name=UNISTR('Registro de proveedores con informaci\00F3n inconsistente') AND INSTR(sample_name,UNISTR('\00F3'))>0 AND INSTR(sample_name,UNISTR('\00BF'))=0 THEN 'PASS' ELSE 'FAIL' END);
END;
/
