...
BEGIN
  ...
  l_parity := 0;
  FOR x IN (SELECT e.EVA_DATOS_JSON, r.RIE_CODIGO, p.PROY_AREA_PRINCIPAL,
              ...
              JOIN RL_MR_PROYECCIONES_EVALUACION p ON p.PROY_EVALUACION_ID=e.EVA_ID) LOOP
    IF json_has(x.EVA_DATOS_JSON,'codigo_riesgo',x.RIE_CODIGO)=0 OR
       ...
       json_has(x.EVA_DATOS_JSON,'estado',x.PROY_ESTADO_EVALUACION)=0 THEN
      l_parity := l_parity + 1;
    END IF;
  END LOOP;
  ...
  DBMS_OUTPUT.PUT_LINE('PROJECTION_JSON_PARITY_IMPLEMENTATION=EXACT');
