...
DECLARE
  ...
  v_diffs NUMBER;
BEGIN
  SELECT COUNT(*) INTO v_backup_rows FROM RL_MR_RIES_NOM_BKP_20260924;
  ...
  SELECT COUNT(*) INTO v_backup_duplicates FROM
   (SELECT RIE_ID, RIE_CODIGO FROM RL_MR_RIES_NOM_BKP_20260924 GROUP BY RIE_ID, RIE_CODIGO HAVING COUNT(*) > 1);
  ...
  UPDATE RL_MR_RIESGOS r
     SET RIE_NOMBRE = (SELECT b.RIE_NOMBRE FROM RL_MR_RIES_NOM_BKP_20260924 b
                        WHERE b.RIE_ID = r.RIE_ID AND b.RIE_CODIGO = r.RIE_CODIGO)
   WHERE EXISTS (SELECT 1 FROM RL_MR_RIES_NOM_BKP_20260924 b
                  WHERE b.RIE_ID = r.RIE_ID AND b.RIE_CODIGO = r.RIE_CODIGO)
     AND (RIE_NOMBRE <> (SELECT b.RIE_NOMBRE FROM RL_MR_RIES_NOM_BKP_20260924 b
                          WHERE b.RIE_ID = r.RIE_ID AND b.RIE_CODIGO = r.RIE_CODIGO)
          OR RIE_NOMBRE IS NULL
          OR (RIE_NOMBRE IS NOT NULL AND (SELECT b.RIE_NOMBRE FROM RL_MR_RIES_NOM_BKP_20260924 b
                                           WHERE b.RIE_ID = r.RIE_ID AND b.RIE_CODIGO = r.RIE_CODIGO) IS NULL));
  v_restored := SQL%ROWCOUNT;
  SELECT COUNT(*) INTO v_diffs
    FROM RL_MR_RIESGOS r JOIN RL_MR_RIES_NOM_BKP_20260924 b
