...
DECLARE
    ...
    END;
BEGIN
    exigir(UPPER(SYS_CONTEXT('USERENV','CURRENT_SCHEMA')) = 'RIESGO_LAVADO', -21701,
           'CURRENT_SCHEMA debe ser RIESGO_LAVADO.');

    SELECT COUNT(*) INTO v_count FROM USER_TABLES WHERE TABLE_NAME = 'RL_AUDITORIA';
    exigir(v_count = 1, -21702, 'No existe RL_AUDITORIA.');
    SELECT COUNT(*) INTO v_count FROM USER_SEQUENCES WHERE SEQUENCE_NAME = 'SEQ_RL_AUDITORIA';
    exigir(v_count = 1, -21703, 'No existe SEQ_RL_AUDITORIA.');

    SELECT MIN(USR_ID) INTO v_usuario_id FROM RL_USUARIOS WHERE USR_ACTIVO = 1;
    exigir(v_usuario_id IS NOT NULL, -21704, 'No existe usuario institucional activo para la prueba.');

    SELECT COUNT(*) INTO v_count FROM RL_MR_RIESGOS WHERE RIE_CODIGO = c_codigo;
    exigir(v_count = 0, -21705, 'Existe un registro previo con el código reservado de prueba; no se continuará.');
    ...
    BEGIN
        ...
        INSERT INTO RL_AUDITORIA (
            ...
            'RL_MR_RIESGOS', TO_CHAR(v_riesgo_id), 'F11_RB',
            NULL, '{"prueba":"rollback_controlado"}', v_usuario_id, NULL,
            NULL, SYSDATE, 'MatricesRiesgos'
        );

        RAISE_APPLICATION_ERROR(-21750, 'FALLO CONTROLADO FASE 11 BLOQUE 6');
    EXCEPTION
        WHEN OTHERS THEN
            v_error := SQLCODE;
            ROLLBACK;
            IF v_error <> -21750 THEN
                RAISE;
            END IF;
    END;

    SELECT COUNT(*) INTO v_count FROM RL_MR_RIESGOS WHERE RIE_CODIGO = c_codigo;
    exigir(v_count = 0, -21706, 'ROLLBACK FALLIDO: persistió el riesgo efímero.');

    SELECT COUNT(*) INTO v_count
      FROM RL_AUDITORIA
     WHERE AUD_TABLA = 'RL_MR_RIESGOS'
       AND AUD_ACCION = 'F11_RB'
       AND AUD_DATOS_NVO LIKE '%rollback_controlado%';
    exigir(v_count = 0, -21707, 'ROLLBACK FALLIDO: persistió la auditoría efímera.');

    SELECT COUNT(*) INTO v_count
      FROM USER_OBJECTS
     WHERE (OBJECT_NAME LIKE 'RL_MR_%' OR OBJECT_NAME = 'RL_AUDITORIA')
       AND STATUS <> 'VALID';
    exigir(v_count = 0, -21708, 'Existen objetos funcionales inválidos.');

    SELECT COUNT(*) INTO v_count
      FROM USER_CONSTRAINTS
     WHERE TABLE_NAME LIKE 'RL_MR_%'
       AND STATUS <> 'ENABLED';
    exigir(v_count = 0, -21709, 'Existen restricciones RL_MR_* deshabilitadas.');
