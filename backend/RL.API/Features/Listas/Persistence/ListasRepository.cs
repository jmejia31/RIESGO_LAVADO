...
namespace RL.API.Features.Listas.Persistence
{
    public class ListasRepository : IListasRepository
    {
        ...
        private static string ConstruirConsultaMonitoreoJuridicas() => @"
            WITH POSITIVOS_AGG AS (
            ...
            ), REPORTE_JURIDICAS AS (
                SELECT NUMERO_PATRONO,
                       LISTA_CONCIDENCIA,
                       FECHA_ENCONTRO,
                       FECHA_CALIFICO
                FROM DNP_IHSS.REPORTE_COINCIDENCIAS
                WHERE TIPO_CALIFICACION_ID = 1
                  AND NUMERO_PATRONO IS NOT NULL
            ), EMPRESA_FUENTE AS (
                SELECT P.RTN,
                       P.NOMBRE,
                       P.NUMEPATRO,
                ...
                SELECT D.RTN, D.NOMBRE, D.NUMEPATRO, D.LISTA_CONCIDENCIA, D.FECHA_ENCONTRO, D.FECHA_CALIFICO,
                       CASE
