...
- Git: `HEAD=origin/desarrollo=99337b4`, ahead=0, behind=0. Se preservaron sin tocar tres untracked preexistentes: `.vscode/`, `agosto_rest.txt` y el PDF de requisitos. `agosto_capturas/` no está presente; se clasifica `PREEXISTING_ENVIRONMENT_DRIFT=1`, `INTERVENTION_ATTRIBUTION=NONE`, sin P0/P1, contaminación ni blocker. `WORKTREE_TRACKED_CLEAN=TRUE`, `STAGED_CHANGES=0`, `UNEXPECTED_UNTRACKED=0`, `PENDING_312_FILES=0`, `SCOPE_WORKTREE_CLEAN=TRUE`.
- Punto de continuación: publicar el commit documental de esta recertificación, verificar el workflow remoto correspondiente al SHA final y detenerse con `SUBFASE_3_1_2=CERRADA/RECERTIFICADA`; `3.1.3` queda habilitada pero no iniciada.
