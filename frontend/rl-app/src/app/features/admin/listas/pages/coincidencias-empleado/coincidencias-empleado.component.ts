...
import { CoincidenciaEmpleadoDetalle, CoincidenciaEmpleadoResumen, Paginado } from '../../models/listas.models';
...
export class CoincidenciasEmpleadoComponent implements OnInit {
  ...
  cargarResumen() {
    ...
    this.listasService.getResumenCoincidenciasEmpleadoPaginado({ pagina: this.paginaActual(), tamanoPagina: this.registrosPorPagina(), buscar: this.buscarTerm() }).subscribe({
      next: (datos) => {
  ...
  private cargarDetalle(): void {
    ...
    this.cargandoDetalle.set(true);
    this.listasService.getDetalleCoincidenciasEmpleadoPaginado({ fecha: this.fechaSeleccionada(), pagina: this.paginaDetalleActual(), tamanoPagina: this.registrosDetallePorPagina(), buscar: this.buscarTermDetalle(), calificacion: this.filtroCalificacion() === 'Todas' ? '' : this.filtroCalificacion() }).subscribe({
      next: (datos) => {
            ...
            next: () => {
              Swal.default.close();
                void this.reportPreview.openExcel(
                  wb,
                  fileName,
                  'Vista previa del reporte de coincidencias de empleados',
